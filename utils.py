...
def extract_fullscene(video_dir, save_dir, num_images=10, image_size = (112, 112)):
...
def extract_face(image_dir, save_dir, image_size = (112,112)):
    detector = MTCNN()
    ...
    for folder_name in tqdm(os.listdir(image_dir)):
        ...
        for filename in os.listdir(folder):
            ...
            fake_image = np.ones((112,112,3)) # white image 
...
def extract_face_from_video(video_dir, save_dir, num_images=10, image_size=(112, 112)):
    detector = MTCNN()
...
def read_ocean_data(video_name, annotation):
    ...
    try:
        ...
        print(f"Warning: Annotation not found for {video_name}. Skipping...")
        return None

def load_images(data_dir, annotation, image_size=(112,112), batch_size = 8):
    X = []
    y = []
